...
class MusiclineScraper
  ...
  def recent_parse
    ...
    base_url = "http://www.musicline.de/de/charts/single?page=0&week=&year=#{current_year}"

    week_nums.each do |week_num|
      url = base_url.gsub('week=', "week=#{week_num}")
      doc = Nokogiri::HTML(open(url))

      if doc.css("tr:nth-child(2) .chart-position.artist-right p a").text.length > 0
        year = Year.find_or_create_by(number: current_year)
